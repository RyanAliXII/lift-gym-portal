...
func CoachAuthMiddleware(next echo.HandlerFunc) echo.HandlerFunc {
	return func(c echo.Context) error {
		s , getSessionErr := session.Get("coach_sid", c)
		if getSessionErr != nil {
			s.Options.MaxAge = -1
			s.Save(c.Request(), c.Response())
			return handleResponseCoach(c)
		}
		if len(s.Values) ==  0{
			s.Options.MaxAge = -1
			s.Save(c.Request(), c.Response())
...
func handleResponseCoach(c echo.Context) error{
	contentType := c.Request().Header.Get("content-type")
	contentTypeWithJSONResponse := []string{"application/x-www-form-urlencoded,application/json"}
	if(	slices.Contains(contentTypeWithJSONResponse, contentType)){
		return c.JSON(http.StatusUnauthorized, map[string]interface{}{
	...
	return c.Redirect(http.StatusFound, "/coaches/login")
}
