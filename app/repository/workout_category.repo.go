...
func(repo * WorkoutCategoryRepository) GetCategories() ( []model.WorkoutCategory, error ) {
	...
	err := repo.db.Select(&categories, `SELECT workout_category.id, workout_category.name, COALESCE(JSON_ARRAYAGG(JSON_OBJECT('id', workout.id, 'name', workout.name, 'description', workout.description, 'imagePath', workout.image_path)), '[]') as workouts from workout_category 
	INNER JOIN category_workout on workout_category.id = category_workout.category_id
	INNER JOIN workout on category_workout.workout_id = workout.id and workout.deleted_at is null
...
func(repo * WorkoutCategoryRepository) GetCategoryById(id int) ( model.WorkoutCategory, error ) {
	...
	err := repo.db.Get(&category, `SELECT workout_category.id, workout_category.name, COALESCE(JSON_ARRAYAGG(JSON_OBJECT('id', workout.id, 'name', workout.name, 'description', workout.description, 'imagePath', workout.image_path)), '[]') as workouts from workout_category 
	INNER JOIN category_workout on workout_category.id = category_workout.category_id
	INNER JOIN workout on category_workout.workout_id = workout.id and workout.deleted_at is null
