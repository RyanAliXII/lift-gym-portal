...
func (repo *UserRepository) GetUserByEmail(email string)(model.User, error) {
	user := model.User{}
	selectQuery := `SELECT user.id, given_name, middle_name, surname, email, COALESCE(role_id, 0) as role_id, password, account.id as account_id, account.is_root FROM user INNER JOIN account on user.account_id = account.id where UPPER(email) = UPPER(?) and deleted_at is null LIMIT 1`
